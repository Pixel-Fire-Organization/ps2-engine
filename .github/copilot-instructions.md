...
- **ISO Generation**:
  ...
  - **Asset Inclusion**: All files located in `app/cd_files/` are automatically pulled into the root of the generated `.iso` filesystem during the build.
  - **Action**: When adding new assets (textures, sounds, scripts) to the project, place them in `app/cd_files/` to ensure they are available to the engine on the PS2 target.

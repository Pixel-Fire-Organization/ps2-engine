...
static int32_t Internal_FindByKey(const char *key) {
  for (int32_t i = 0; i < RES_MAX_ENTRIES; i++) {
    if (s_Entries[i].state != RES_STATE_EMPTY &&
        strncmp(s_Entries[i].key, key, IO_FILE_MAX_PATH) == 0) {
      return i;
    }
  }
  return -1;
...
static int32_t Internal_EvictLRU(void) {
  ...
  if (bestIndex >= 0) {
    Engine_LogInfo("Resource LRU evict: slot %d (%s)", bestIndex,
                   s_Entries[bestIndex].key);
...
static void Internal_UnloadEntry(int32_t index) {
  ...
  for (uint8_t d = 0; d < entry->depCount; d++) {
    ...
    if (depIdx >= 0 && depIdx < RES_MAX_ENTRIES &&
        s_Entries[depIdx].state != RES_STATE_EMPTY) {
...
static void Internal_OnAsyncLoadComplete(void *data, size_t size,
  ...
  if (!data || size == 0) {
    Engine_LogError("Resource async load failed for slot %d (%s)", idx,
                    entry->key);
    entry->state = RES_STATE_EMPTY;
  ...
  case RES_SOUND: {
    ...
    if (wave.data != NULL) {
      ...
      Engine_LogError("Resource: failed to decode sound slot %d (%s)", idx,
                      entry->key);
      entry->state = RES_STATE_EMPTY;
    }
#else
  ...
  case RES_FONT: {
    ...
    if (entry->handle.font.texture.id > 0) {
      ...
      Engine_LogError("Resource: failed to decode font slot %d (%s)", idx,
                      entry->key);
      entry->state = RES_STATE_EMPTY;
    }
  } break;
...
static bool Internal_ParseHeaderAndLoadDeps(const void *data, size_t size,
  ...
  for (uint8_t d = 0; d < entry->depCount; d++) {
    ...
    if (depHandle >= 0) {
      ...
      int32_t newDep =
          Engine_Resource_Load((ResourceType)outHeader->type, outHeader->deps[d]);
      if (newDep >= 0) {
        s_Entries[newDep].refCount++;
        entry->deps[d] = newDep;
      } else {
        Engine_LogError("Resource: failed to load dependency '%s'",
                        outHeader->deps[d]);
        entry->deps[d] = -1;
      }
    }
...
int32_t Engine_Resource_Load(ResourceType type, const char *path) {
  ...
  if (slot < 0) {
    Engine_LogError("Resource: no free slots and nothing evictable for '%s'",
                    path);
    return -1;
  }
  ...
  ResourceLoadContext *ctx =
      (ResourceLoadContext *)Engine_PoolAllocMain();
  if (!ctx) {

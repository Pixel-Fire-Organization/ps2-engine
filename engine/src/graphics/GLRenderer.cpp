...
void GLRenderer::Render()
{
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    ApplyProjection(m_drawLists.GetCamera3D());

    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    ApplyCameraTransform(m_drawLists.GetCamera3D());
...
uint32_t GLRenderer::UploadTexture(const void* pixels, int width, int height, PixelFormat format)
{
    GLuint tex = 0;
    glGenTextures(1, &tex);
    if (tex == 0)

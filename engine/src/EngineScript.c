...
void Engine_Script_Close(void) {
  ...
  s_FrameCount        = 0;
  for (int i = 0; i < SCRIPTING_MAX_CAMERAS_3D; i++) s_Camera3DActive[i] = false;
  for (int i = 0; i < SCRIPTING_MAX_CAMERAS_2D; i++) s_Camera2DActive[i] = false;
}
...
bool Engine_Script_Run(int unitIndex) {
  ...
  if (luaL_loadbuffer(unit->L, code, codeSize, "PS2_Script") != LUA_OK) {
    ...
    Engine_LogError("Lua compile error in slot %d: %s", codeSlot,
                    err ? err : "(unknown)");
  ...
  if (lua_pcall(unit->L, 0, 0, 0) != LUA_OK) {
    const char *err = lua_tostring(unit->L, -1);
    Engine_LogError("Lua runtime error in unit %d: %s", unitIndex,
                    err ? err : "(unknown)");
...
void Engine_Script_FrameTick(void) {
    ...
    for (int i = 0; i < SCRIPTING_MAX_CAMERAS_3D; i++) {
        if (s_Camera3DActive[i] &&
            (s_FrameCount - s_Camera3DLastUsed[i]) >= SCRIPTING_CAM_IDLE_FRAMES_EVICT) {
    ...
    for (int i = 0; i < SCRIPTING_MAX_CAMERAS_2D; i++) {
        if (s_Camera2DActive[i] &&
            (s_FrameCount - s_Camera2DLastUsed[i]) >= SCRIPTING_CAM_IDLE_FRAMES_EVICT) {
...
static int Lua_Graphics_MakeCamera3D(lua_State *L) {
    ...
    int slot = -1;
    for (int i = 0; i < SCRIPTING_MAX_CAMERAS_3D; i++) {
        if (!s_Camera3DActive[i]) { slot = i; break; }
    }
    if (slot < 0) {
    ...
    Camera3D cam;
    cam.position   = (Vector3){ (float)luaL_checknumber(L, 1),
                                 (float)luaL_checknumber(L, 2),
                                 (float)luaL_checknumber(L, 3) };
    cam.target     = (Vector3){ (float)luaL_checknumber(L, 4),
                                 (float)luaL_checknumber(L, 5),
                                 (float)luaL_checknumber(L, 6) };
    cam.up         = (Vector3){ (float)luaL_checknumber(L, 7),
...
static int Lua_Graphics_MakeCamera2D(lua_State *L) {
    int slot = -1;
    for (int i = 0; i < SCRIPTING_MAX_CAMERAS_2D; i++) {
        if (!s_Camera2DActive[i]) { slot = i; break; }
    }
    if (slot < 0) {
...
static int Lua_Graphics_DrawCubeTextured(lua_State *L) {
  ...
  Color tint = WHITE;
  if (lua_istable(L, 6)) {
    lua_geti(L, 6, 1); unsigned char r = (unsigned char)lua_tointeger(L, -1);
    lua_geti(L, 6, 2); unsigned char g = (unsigned char)lua_tointeger(L, -1);
    lua_geti(L, 6, 3); unsigned char b = (unsigned char)lua_tointeger(L, -1);
    lua_geti(L, 6, 4); unsigned char a = (unsigned char)lua_tointeger(L, -1);
    lua_pop(L, 4);
    tint = (Color){r, g, b, a};
  }
  ...
  rlBegin(RL_QUADS);
    ...
    rlTexCoord2f(0.0f, 0.0f); rlVertex3f(px - h, py - h, pz + h);
    rlTexCoord2f(1.0f, 0.0f); rlVertex3f(px + h, py - h, pz + h);
    rlTexCoord2f(1.0f, 1.0f); rlVertex3f(px + h, py + h, pz + h);
    rlTexCoord2f(0.0f, 1.0f); rlVertex3f(px - h, py + h, pz + h);
...
static int Lua_Input_IsPadPressed(lua_State *L) {
  ...
  // D-pad
  else if (strcmp(btn, "dpad_up")    == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_FACE_UP);
  else if (strcmp(btn, "dpad_down")  == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_FACE_DOWN);
  else if (strcmp(btn, "dpad_left")  == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_FACE_LEFT);
  else if (strcmp(btn, "dpad_right") == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_FACE_RIGHT);
  // Shoulder buttons
  else if (strcmp(btn, "l1")         == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_TRIGGER_1);
  else if (strcmp(btn, "l2")         == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_TRIGGER_2);
  else if (strcmp(btn, "r1")         == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_RIGHT_TRIGGER_1);
  else if (strcmp(btn, "r2")         == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_RIGHT_TRIGGER_2);
  // Stick clicks
  else if (strcmp(btn, "l3")         == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_THUMB);
  else if (strcmp(btn, "r3")         == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_RIGHT_THUMB);
  // Menu
  else if (strcmp(btn, "start")      == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_MIDDLE_RIGHT);
  else if (strcmp(btn, "select")     == 0) pressed = IsGamepadButtonDown(0, GAMEPAD_BUTTON_MIDDLE_LEFT);
  else Engine_LogError("[Lua] input.is_pad_pressed: unknown button '%s'", btn);
...
static void RegisterIOBindings(lua_State *L) {
  lua_newtable(L);
  lua_pushcfunction(L, Lua_IO_Open);      lua_setfield(L, -2, "open");
  lua_pushcfunction(L, Lua_IO_OpenWrite); lua_setfield(L, -2, "open_write");
  lua_pushcfunction(L, Lua_IO_Close);     lua_setfield(L, -2, "close");
  lua_pushcfunction(L, Lua_IO_GetSize);   lua_setfield(L, -2, "get_size");
  lua_pushcfunction(L, Lua_IO_Read);      lua_setfield(L, -2, "read");
  lua_pushcfunction(L, Lua_IO_Write);     lua_setfield(L, -2, "write");
  lua_setglobal(L, "io");
}
...
static int Lua_Resources_Load(lua_State *L) {
  ...
  if      (strcmp(typeStr, "TEXTURE") == 0) type = RES_TEXTURE;
  else if (strcmp(typeStr, "MODEL")   == 0) type = RES_MODEL;
  else if (strcmp(typeStr, "SOUND")   == 0) type = RES_SOUND;
  else if (strcmp(typeStr, "FONT")    == 0) type = RES_FONT;
  else Engine_LogError("[Lua] resources.load: unknown type '%s'", typeStr);
...
static void RegisterResourceBindings(lua_State *L) {
  lua_newtable(L);
  lua_pushcfunction(L, Lua_Resources_Load);    lua_setfield(L, -2, "load");
  lua_pushcfunction(L, Lua_Resources_Unload);  lua_setfield(L, -2, "unload");
  lua_pushcfunction(L, Lua_Resources_IsReady); lua_setfield(L, -2, "is_ready");
...
static void RegisterLevelBindings(lua_State *L) {
  lua_newtable(L);
  lua_pushcfunction(L, Lua_Level_Load);   lua_setfield(L, -2, "load");
  lua_pushcfunction(L, Lua_Level_Unload); lua_setfield(L, -2, "unload");
